...
function desktop.lookup_file_icon(filename)
	...
	-- If we dont find ad icon, then pretend is a plain text file
...
function desktop.add_icons(args)
	...
	-- Trying to fallback on Adwaita if theme.icon_theme is not defined if its missing, no icons will be shown
	if not theme.icon_theme then
		theme.icon_theme=args.icon_theme or 'Obsidian Green'
	end
